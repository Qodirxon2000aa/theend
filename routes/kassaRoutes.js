...
const Admin = require('../models/Kassa'); 
...
/**
 * GET /kassa – Barcha adminlarni olish
 */
...
/**
 * GET /kassa/:id – Bitta adminni olish
 */
...
/**
 * PUT /kassa/:id – Adminni yangilash
 */
...
/**
 * DELETE /kassa/:id – Adminni o‘chirish
 */
...
/**
 * POST /kassa/login – Admin tizimga kirishi
 */
router.post('/login', async (req, res) => {
  try {
    ...
    // Parolsiz foydalanuvchi ma’lumotini qaytaramiz
    const { password: _, ...adminData } = admin.toObject();

    res.json({ message: 'Tizimga kirish muvaffaqiyatli', admin: adminData });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server xatosi' });
  }
